(function(){
  ...
  function mealHtml(id,second=false){
    ...
    const img=m.image_url?'<img class="kac-img" loading="lazy" src="'+esc(m.image_url)+'" alt="'+esc(m.name)+'">':'<span class="kac-ph">🍽️</span>';
  ...
  function localValue(keys){
    try{for(const k of keys){const v=localStorage.getItem(k);if(v)return portionLabel(v)}}catch(e){}
    return '';
  }
  function portionFor(userId,g,sel){
    ...
    for(const k of keys){if(sel?.[k])return portionLabel(sel[k])}
    if(g===3){
      const lv=localValue(['kraw_group3_portion_1_'+userId+'_'+(S?.today||'today')]);
      if(lv)return lv;
    }
    return 'Normal';
  }
  ...
  function secondPortion(userId,g){
    ...
    for(const k of keys){if(r[k])return portionLabel(r[k])}
    const today=S?.today||'today';
    if(g===1){
      const lv=localValue(['kraw_second_portion_'+userId+'_'+today,'kraw_admin_second_portion_'+today+'_'+userId]);
      if(lv)return lv;
    }
    if(g===3){
      const lv=localValue(['kraw_group3_portion_2_'+userId+'_'+today]);
      if(lv)return lv;
    }
    return 'Normal';
  }
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='kacStyle';s.textContent=`
      #ov .kac-meal{display:flex;align-items:center;gap:8px;min-height:40px;margin:2px 0;font-size:12px;white-space:normal}
      #ov .kac-meal img,#ov .kac-ph{width:40px;height:40px;object-fit:cover;border-radius:7px;flex:0 0 40px;background:#eef3f8;display:grid;place-items:center}
      ...
      #ov .kac-portion{font-size:11px;font-weight:900;color:#dc2626;margin:2px 0 4px 48px;min-height:14px}
      #ov .kac-empty{color:#9aa6b2;padding:8px 0}
      #ov td[data-kac-group]{min-width:165px;vertical-align:top!important;padding-top:8px!important;padding-bottom:8px!important}
    `;document.head.appendChild(s);
  }
  function bindImageFallbacks(root){
    root.querySelectorAll('img.kac-img').forEach(img=>{
      img.onerror=()=>{const ph=document.createElement('span');ph.className='kac-ph';ph.textContent='🍽️';img.replaceWith(ph)};
    });
  }
  function patch(){
    if(patching)return;
    try{
      ...
      [...table.querySelectorAll('tr')].slice(1).forEach((tr,i)=>{
        ...
        [1,2,3,4].forEach(g=>{
          ...
          const p1=(firstId&&g!==4)?portionFor(u.id,g,sel):'';
          const p2=second?secondPortion(u.id,g):'';
          let html='';
  ...
  const mo=new MutationObserver(()=>{if(S?.user?.role==='admin')setTimeout(patch,40)});
