(function(){
  ...
  function hasComplete(){return !!(S?.user?.role==='personel'&&P?.[1]&&P?.[2]&&P?.[3]&&P?.[4])}
  function hasSaved(){return !!S?.ownSelection}
  function editLocked(){const n=trNow();return hasSaved()&&n.hour>=11}
  function sig(){return [P?.[1]||'',window.D2?.[1]||'',P?.[2]||'',P?.[3]||'',window.D2?.[3]||'',P?.[4]||'',hasSaved()?'1':'0',editLocked()?'1':'0'].join('|')}
  ...
  function style(){
    ...
    const s=document.createElement('style');s.id='krawPersonSummaryStyle';s.textContent=`
      ...
      .kraw-side-summary-head{padding:13px 14px;background:#122b47;border-bottom:1px solid #2a4d70;display:flex;justify-content:space-between;align-items:center;gap:10px}.kraw-side-summary-head b{font-size:16px}.kraw-side-summary-head small{color:#a9bdd2}
  ...
  function render(){
    try{
      ...
      if(!hasComplete()){card.style.display='none';lastSig='';return}
      card.style.display='block';
      const locked=editLocked();
      const x={1:P?.[1],2:P?.[2],3:P?.[3],4:P?.[4]};
      card.innerHTML=`<div class="kraw-side-summary-head"><div><b>📋 Seçtiklerim</b><small>${hasSaved()?'Kayıtlı sipariş':'Henüz onaylanmadı'}</small></div></div>`+
        [1,2,3,4].map(g=>{const s2=(g===1||g===3)?window.D2?.[g]:null;return `<div class="kraw-side-row"><div class="kraw-side-no">${g}</div><div><div class="kraw-side-meal">${escP(mealName(x[g]))}</div>${s2?`<div class="kraw-side-meal second">+ ${escP(mealName(s2))}</div>`:''}<div class="kraw-side-label">${LABELS[g]}</div></div></div>`}).join('')+
  ...
  // 11:00 sonrası mevcut kayıtlı sipariş üzerinde değişiklik yapılmasını arayüzde engelle.
  ...
  document.addEventListener('click',e=>{
    if(typeof S==='undefined'||S.user?.role!=='personel')return;
    if(e.target.closest?.('.meal,#saveBtn,#krawOrderConfirmBtn,.kraw-final-confirm button'))setTimeout(render,120);
  },false);

  document.addEventListener('visibilitychange',()=>{if(!document.hidden)render()});
  let tries=0;const t=setInterval(()=>{tries++;render();if(S?.user?.role==='personel'&&document.querySelector('.guesthead'))clearInterval(t);if(tries>50)clearInterval(t)},120);
  setInterval(()=>{if(sig()!==lastSig)render()},30000);
