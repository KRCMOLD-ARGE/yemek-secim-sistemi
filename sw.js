const CACHE='kraw-yemek-v32';
const SHELL=['./','./index.html','./app.html','./manifest.webmanifest','./icon.svg','./kraw-wallpaper.png','./order-list.js','./double-select.js','./all-meals-fix.js','./admin-group4.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.html')){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      const scripts=['order-list.js','double-select.js','all-meals-fix.js','admin-group4.js'];
      scripts.forEach(name=>{if(!html.includes(name))html=html.replace('</body>','<script src="./'+name+'?v=32"></script></body>')});
  ...
  if(isIndex){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      html=html.replace('src="app.html"','src="app.html?v=32"');
  ...
  const fresh=['/order-list.js','/double-select.js','/all-meals-fix.js','/admin-group4.js'].some(x=>url.pathname.endsWith(x));
