...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.html')){event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{let html=await res.text();if(!html.includes('order-list.js'))html=html.replace('</body>','<script src="./order-list.js?v=22"></script></body>');return new Response(html,{status:res.status,statusText:res.statusText,headers:res.headers})}).catch(()=>caches.match('./app.html')));return;}
