const CACHE='kraw-yemek-v37';
const SHELL=['./','./index.html','./app.html','./manifest.webmanifest','./icon.svg','./kraw-wallpaper.png','./order-list.js','./all-meals-fix.js','./admin-group4.js','./group-tabs.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.html')){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      const scripts=['order-list.js','all-meals-fix.js','admin-group4.js','group-tabs.js'];
      scripts.forEach(name=>{if(!html.includes(name))html=html.replace('</body>','<script src="./'+name+'?v=37"></script></body>')});
  ...
  if(isIndex){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      html=html.replace('src="app.html"','src="app.html?v=37"');
  ...
  const fresh=['/order-list.js','/all-meals-fix.js','/admin-group4.js','/group-tabs.js'].some(x=>url.pathname.endsWith(x));
