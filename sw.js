...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.html')){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      scripts.forEach(name=>{if(!html.includes(name))html=html.replace('</body>','<script src="./'+name+'?v=74"></script></body>')});
  ...
  if(isIndex){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      html=html.replace('src="app.html"','src="app.html?v=74"');
