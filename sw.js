const CACHE='kraw-yemek-v29';
const SHELL=['./','./index.html','./app.html','./manifest.webmanifest','./icon.svg','./kraw-wallpaper.png','./order-list.js','./double-select.js','./all-meals-fix.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app.html')){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      if(!html.includes('order-list.js'))html=html.replace('</body>','<script src="./order-list.js?v=29"></script><script src="./double-select.js?v=29"></script><script src="./all-meals-fix.js?v=29"></script></body>');
      else {
        if(!html.includes('double-select.js'))html=html.replace('</body>','<script src="./double-select.js?v=29"></script></body>');
        if(!html.includes('all-meals-fix.js'))html=html.replace('</body>','<script src="./all-meals-fix.js?v=29"></script></body>');
      }
  ...
  if(isIndex){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async res=>{
      ...
      html=html.replace('src="app.html"','src="app.html?v=29"');
  ...
  const fresh=url.pathname.endsWith('/order-list.js')||url.pathname.endsWith('/double-select.js')||url.pathname.endsWith('/all-meals-fix.js');
