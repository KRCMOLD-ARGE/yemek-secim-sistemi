(function(){
  ...
  function cleanPortionControls(){
    try{[3,4].forEach(g=>{const group=document.querySelector('.group[data-g="'+g+'"]');if(!group)return;group.querySelectorAll('.portiontag,.portionmini,.portionchoice').forEach(el=>el.remove());group.querySelectorAll('button').forEach(btn=>{const t=(btn.textContent||'').trim();if(t==='Az'||t==='Normal')btn.remove()})})}catch(e){}
  }
  ...
  function hookSave(){
    try{
      if(typeof saveSel==='function'&&!saveSel.__savedOrderHook){const original=saveSel;saveSel=async function(){await original();setTimeout(()=>draw(false),450)};saveSel.__savedOrderHook=true}
      if(typeof closeSuccess==='function'&&!closeSuccess.__savedOrderHook){const originalClose=closeSuccess;closeSuccess=function(){originalClose();setTimeout(()=>draw(true),120)};closeSuccess.__savedOrderHook=true}
    }catch(e){}
  }
