(function(){
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id='krawOrderListStyle';s.textContent=`
      ...
      .kraw-order-actions{padding:14px 18px 18px;display:grid;grid-template-columns:1fr 1fr;gap:10px}.kraw-order-confirm{width:100%;border:0;border-radius:11px;padding:14px 16px;color:#fff;font-size:17px;font-weight:900;cursor:pointer}.kraw-order-create{background:#16965a}.kraw-order-update{background:#2869dc}.kraw-order-confirm:disabled{opacity:.6;cursor:not-allowed}
      ...
      @media(max-width:600px){.kraw-order-card{width:96vw}.kraw-order-head h3{font-size:20px}.kraw-order-meal{font-size:15px}.kraw-order-user{font-size:16px}.kraw-top-notice{top:8px}.kraw-order-actions{grid-template-columns:1fr}}
  ...
  function showTopNotice(mode='create'){ensureStyle();document.getElementById('krawTopNotice')?.remove();const n=document.createElement('div');n.id='krawTopNotice';n.className='kraw-top-notice';const isUpdate=mode==='update';n.innerHTML='<div class="ico">✓</div><div><b>'+(isUpdate?'Siparişiniz güncellendi':'Siparişiniz oluşturuldu')+'</b><span>Yemek seçimleriniz başarıyla kaydedildi. Sipariş listeniz açıldı.</span></div><button type="button" class="kraw-top-notice-close" aria-label="Bildirimi kapat">×</button>';n.querySelector('.kraw-top-notice-close').onclick=hideTopNotice;document.body.appendChild(n);requestAnimationFrame(()=>n.classList.add('show'));clearTimeout(toastTimer);toastTimer=setTimeout(hideTopNotice,4500)}
  ...
  async function confirmOrder(mode='create'){
    if(confirming||!ready())return;
    const btn=document.getElementById(mode==='update'?'krawOrderUpdateBtn':'krawOrderCreateBtn')||document.getElementById('saveBtn');
    const all=[document.getElementById('krawOrderCreateBtn'),document.getElementById('krawOrderUpdateBtn')].filter(Boolean);
    confirming=true;all.forEach(b=>b.disabled=true);if(btn)btn.textContent='⏳ Kaydediliyor...';
    try{
      const x=ids();
      ...
      manuallyClosed=false;draw(true,true);showTopNotice(mode);
