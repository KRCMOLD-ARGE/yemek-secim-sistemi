(function(){
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id='krawOrderListStyle';s.textContent=`
      ...
      .kraw-top-notice{position:fixed;z-index:13000;top:12px;left:50%;transform:translate(-50%,-140%);width:min(620px,calc(100vw - 24px));background:#fff;color:#10233b;border:1px solid #dce5ef;border-radius:14px;box-shadow:0 14px 45px rgba(0,0,0,.28);padding:13px 16px;display:flex;align-items:center;gap:12px;transition:transform .28s ease,opacity .28s ease;opacity:0}
      .kraw-top-notice.show{transform:translate(-50%,0);opacity:1}.kraw-top-notice .ico{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:#e7f7ee;color:#15824d;font-size:20px;flex:0 0 auto}.kraw-top-notice b{display:block;font-size:14px}.kraw-top-notice span{display:block;color:#65758a;font-size:12px;margin-top:2px}
  ...
  function showTopNotice(){
    ...
    n.innerHTML='<div class="ico">✓</div><div><b>Yemek seçimleriniz tamamlandı</b><span>Listeyi kontrol edip Siparişi Onayla butonuna basın.</span></div>';
    document.body.appendChild(n);
    requestAnimationFrame(()=>n.classList.add('show'));
    clearTimeout(toastTimer);toastTimer=setTimeout(()=>{n.classList.remove('show');setTimeout(()=>n.remove(),300)},3200);
  }
  ...
  function draw(force=false,saved=false){
    try{
      ...
      card.innerHTML=`<button type="button" class="kraw-order-close" aria-label="Kapat">×</button><div class="kraw-order-head"><h3>📋 Sipariş Listem</h3><div class="kraw-order-user">👤 ${esc2(S.user.full_name)}</div></div>`+
        ...
        (saved?'<div class="kraw-order-ok">✅ Siparişiniz kaydedildi. İsterseniz seçimleri değiştirip tekrar onaylayabilirsiniz.</div>':'<div class="kraw-order-hint">Seçtiğiniz yemekleri kontrol edin. Ardından siparişi onaylayın.</div>')+
  ...
  // 4. grup seçildiğinde, diğer gruplar da tamamsa sipariş listesi otomatik açılır
  // ve telefon bildirimi görünümünde üstten kayan bir uyarı gösterilir.
  document.addEventListener('click',e=>{
    const meal=e.target.closest?.('.meal');if(!meal)return;
    const g=Number(meal.closest('.group[data-g]')?.dataset?.g||0);if(g!==4)return;
    setTimeout(()=>{
      if(!ready())return;
      manuallyClosed=false;
      showTopNotice();
